...
from constants import CLABEL, EXTENSION, FTYPE, LABEL, MLABEL, TEMPLATES
from formatting import format_plot

data_dir = ('/Users/tylerpennebaker/Library/CloudStorage/Box-Box/'
            'Elias-Raphaële shared folder/LGES project/WP6/Cell cycling/data/')
save_dir = ('/Users/tylerpennebaker/Library/CloudStorage/Box-Box/'
            'Elias-Raphaële shared folder/LGES project/WP6/Cell cycling/data/')
fig_save_dir = ('/Users/tylerpennebaker/Library/CloudStorage/Box-Box/Elias-'
                'Raphaële shared folder/LGES project/WP6/Cell cycling/'
                'figures/')
summary_save_dir = ('/Users/tylerpennebaker/Library/CloudStorage/Box-Box/Elias'
                    '-Raphaële shared folder/LGES project/WP6/Cell cycling/'
                    'data/')
...
def get_active_mass(cell_name):
    """
    Given a cell name, loads the shared echem excel sheet and finds the entered
    active mass (in g) of the cell.
    ...
    active_mass = 1
    df = pd.read_excel('/Users/tylerpennebaker/Library/CloudStorage/Box-Box/'
                       'Elias-Raphaële shared folder/LGES project/WP6/Cell '
                       'cycling/cell_cycling_new/cyclingdatalog.xlsx')
...
def get_molar_mass(cell_name):
    ...
    molar_mass = 1
    df = pd.read_excel('/Users/erick/Library/CloudStorage/Box-Box/'
                       'Electrochem/shared echem notebook.xlsx')
    sample_info = df[df['Sample'] == cell_name]
    if sample_info.empty:
        # Handle the case when there are no rows matching the condition
        print(f"No information found for {cell_name}")
...
def get_voltage_window(df: pd.DataFrame):
    """
    Given a standardized dataframe, get the upper and lower voltage cutoffs of
    the first cycle, rounded to 2 decimal points.

    PARAMETERS:
        df: a pandas Dataframe object
            a dataframe containing standardized battery cycling data
    RETURNS: tuple of floats, (lower_cutoff, upper_cutoff)
        the upper and lower voltage cutoffs of the first cycle,
        (upper_cutoff, lower_cutoff)
    """

    first_cycle_df = df[df[LABEL.CYCLE_INDEX.value] == 1]
    charge_df = first_cycle_df[
            first_cycle_df[LABEL.STEP_TYPE.value] == 'charge']
    discharge_df = first_cycle_df[
            first_cycle_df[LABEL.STEP_TYPE.value] == 'discharge']
    if charge_df.empty or discharge_df.empty:
        return (None, None)
    upper_cutoff = np.round(charge_df[LABEL.VOLTAGE.value].iloc[-1], 2)
    lower_cutoff = np.round(discharge_df[LABEL.VOLTAGE.value].iloc[-1], 2)
...
def get_combined_capacity(df: pd.DataFrame):
    """
    Given a data row of battery cycling data, determine the value of the
    combined capacity column.

    PARAMETERS:
        series: a pandas Series object
            row containing battery cycling data
    RETURNS: float
        the value of the combined capacity column, to be used to calculate a
        new column
    """
...
def get_step_type(series):
    """
    Given a data row of battery cycling data, determine the step type of the
    row.
    ...
    if LABEL.CONTROL_CURR.value in series.index:
        ...
        current = series[LABEL.CURRENT.value]
...
def get_cycle_status(df: pd.DataFrame, voltage_window):
    """
    Given the charge or discharge step of a single cycle as a dataframe,
    determine whether that charge or discharge step has completed based on
    the voltage window.

    PARAMETERS:
        df: a pandas Dataframe object
            a subset dataframe of standardized battery cycling data containing
            charge or discharge data for a single cycle
    ...
    RETURNS: str
        A string of either 'not started', 'complete', or 'incomplete', based
        on the status of the cycle step
    """
...
def standardize(df: pd.DataFrame, filetype, metadata_df: pd.DataFrame = None):
    """
    Given a parsed dataframe containing battery cycling data, reads, processes,
    and standardizes the data to a universal format.

    PARAMETERS:
        df: pandas Dataframe object
            dataframe containing cycling data
        filetype: str, default None
            The type of data file data originated from. Must match a defined
            filetype ('arbin', 'biologic', 'universal_csv')
    RETURNS: pandas Dataframe
        Returns a dataframe object containing data in a standardized format
        (i.e. column names/ units standardized, certain columns added if
        necessary)
    """
    ...
    if LABEL.CAP.value not in df.columns:
        df[LABEL.CAP.value] = df.apply(lambda x: get_combined_capacity(x),
                                       axis=1)
    # calculate dV
    df[LABEL.DV.value] = df[LABEL.VOLTAGE.value].diff()
    ...
    if (isinstance(metadata_df, pd.DataFrame)
            and MLABEL.CHAR_MASS.value in metadata_df.index):
        ...
        if char_mass:
            ...
            # Wh/kg
            df[LABEL.GRAV_ENERGY_C.value] = (df[LABEL.ENERGY_C.value]
                                             / char_mass * 1e6)
            df[LABEL.GRAV_ENERGY_D.value] = (df[LABEL.ENERGY_D.value]
                                             / char_mass * 1e6)
            # dQ/dV, mAh/g/V
            df[LABEL.GRAV_DQ_DV.value] = (df[LABEL.DQ_DV.value]
                                          / char_mass * 1000)
            # rate, mA/g
            df[LABEL.RATE.value] = (np.abs(df[LABEL.CURRENT.value]) * 1000
                                    / (char_mass / 1000))
            if MLABEL.THEORETICAL_CAP.value in metadata_df.index:
                theor_cap = metadata_df.loc[MLABEL.THEORETICAL_CAP.value,
                                            'value']
                if theor_cap:
                    df[LABEL.C_RATE.value] = (1e6
                                              * np.abs(df[LABEL.CURRENT.value])
                                              / theor_cap / char_mass)

    if (isinstance(metadata_df, pd.DataFrame)
            and MLABEL.CHAR_VOL.value in metadata_df.index):
...
def get_csv(cell_name, filetype=None, metadata=None, save_csv=None):
    ...
    if Path(filepath).suffix == '.mpt':
        ...
        raise ValueError('{} does not have a recognized filetype. '
                         'List of valid filetypes: {}'.format(
                            Path(filepath).name, [x.value for x in EXTENSION]))
    ...
    if filetype == FTYPE.BIOLOGIC.value:
        ...
        for i, line in enumerate(lines):
            # data starts in the line that contains the phrase
            # "charge/discharge"
            if re.search('charge/discharge', line):
                start_line = i
        df = pd.read_csv(filepath, skiprows=start_line, header=0, sep="\t",
                         encoding='ISO-8859-1')
    ...
    # Case for Arbin .res file
        ...
        # This won't work becasue i don't know what "echem" is
        # (from vincent somehow)
...
def get_cell_summary(cell_name, c_savepath=None):
    """
    Given a cell name, calculates summary data for each cycle.
    ...
    PARAMETERS:
        ...
        savepath: str, path object, file-like object, default None
            Path to save cycle summary csv file to. If None, no csv file is
            saved.
    ...
    num_cycles = (df[LABEL.CYCLE_INDEX.value].iloc[-1]
                  - df[LABEL.CYCLE_INDEX.value].iloc[0] + 1)
    ...
    for i in range(num_cycles):
        ...
        if lower_cutoff and upper_cutoff:
            cycle_status_c = get_cycle_status(cycle_c_df,
                                              (lower_cutoff, upper_cutoff))
            cycle_status_d = get_cycle_status(cycle_d_df,
                                              (lower_cutoff, upper_cutoff))
        ...
        if not cycle_c_df.empty:
            ...
            global voltage_c
            voltage_c = (last_c_row[LABEL.ENERGY_C.value]
                         / last_c_row[LABEL.CAP_C.value] * 1000)
        ...
        if not cycle_d_df.empty:
            ...
            global voltage_d
            voltage_d = (last_d_row[LABEL.ENERGY_D.value]
                         / last_d_row[LABEL.CAP_D.value] * 1000)
        ...
        if LABEL.GRAV_CAP_C.value in df.columns:
            if not cycle_c_df.empty:
                ...
                grav_charge_energy.append(
                    last_c_row[LABEL.GRAV_ENERGY_C.value])
            if not cycle_d_df.empty:
                grav_discharge_capacity.append(
                    last_d_row[LABEL.GRAV_CAP_D.value])
                grav_discharge_energy.append(
                    last_d_row[LABEL.GRAV_ENERGY_D.value])
        ...
        if lower_cutoff and upper_cutoff:
            if cycle_status_c == 'complete' and cycle_status_d == 'complete':
                coulombic_efficiency.append(last_d_row[LABEL.CAP_D.value]
                                            / last_c_row[LABEL.CAP_C.value]
                                            * 100)
                voltage_hysteresis.append(voltage_c - voltage_d)
                energy_efficiency.append(last_d_row[LABEL.ENERGY_D.value]
                                         / last_c_row[LABEL.ENERGY_C.value]
                                         * 100)
            else:
                coulombic_efficiency.append(last_d_row[LABEL.CAP_D.value]
                                            / last_c_row[LABEL.CAP_C.value]
...
def plot_VoltProfile(cell_name, plt: pyplot = None, ax: axes.Axes = None,
                     cycles=None, continuous=False, li_per_fu=True,
                     cmap=None, palette='flare', xlabel=None,
                     ylabel='Voltage (V)', xlim=None, ylim=None,
                     line_width=1.5, show_lgd=True, lgd_fsize=20,
    ...
    for i in clean_cycles:
        ...
        # charge_cap can have some zero values that screw up the plot.
        # This replaces 0s with NaNs to fix.
        charge_cap.replace(0, np.nan, inplace=True)
        if continuous:
            discharge_cap = charge_cap.iloc[-1] - discharge_cap
        color = colors[i + 1]
        label = ('Cycle {}'.format(i+1)
                 if (i == first_cycle or i == last_cycle) else None)
        ax.plot(charge_cap, cycle_c_df[LABEL.VOLTAGE.value], '-', color=color,
                lw=line_width, label=label)
        ax.plot(discharge_cap, cycle_d_df[LABEL.VOLTAGE.value], '-',
    ...
    if li_per_fu:
        ...
        def active_to_ion(capacity):
            """
            Converts active material specific capacity to ions
            inserted/extracted.
            """
        ...
        def ion_to_active(capacity):
            """
            Converts alkali ions inserted/extracted to active material specific
            capacity.
            """
            return capacity * single_ion_capacity
        ...
        if ax:
            ...
            ax2 = ax.secondary_xaxis('top',
                                     functions=(active_to_ion, ion_to_active))
            ax2.tick_params(direction=direction, width=tick_width,
                            length=tick_len)
            # secondary minor axis settings
            ax2.xaxis.set_minor_locator(minor_locator)
            ax2.tick_params(which='minor', direction=direction,
                            width=tick_width, length=tick_len/2)
...
def plot_CycvsCap(cell_names, plt: pyplot = None, ax: axes.Axes = None,
                  plot_CE=False, cycles=None, continuous=False,
                  label_names=None, cmap=None, palette='tab10', xlabel=None,
                  ylabel='Specific Capacity (mAh/g)',
                  ce_ylabel='Coulombic Efficiency (%)', xlim=None, ylim=None,
                  ce_ylim=None, line_width=1.5, show_lgd=True, lgd_fsize=20,
                  show_fig=True, savefile=None, save_name=None, fsize=(6, 6)):
    if not plt:
        plt = pyplot
        # set font size
        plt.rcParams['font.size'] = lgd_fsize
    ...
    # If only one cell name is provided, convert it to a list to make it
    # interact with the rest of the code
    ...
    for index in range(len(cell_names)):
        ...
        ax1.plot(cycle_number, capacity, linestyle='-', marker='o',
                 lw=line_width, label=cell_name, color=colors[index])
    ...
    if plot_CE:
        ...
        for index in range(len(cell_names)):
            ...
            cycle_number = df[CLABEL.CYCLE_INDEX.value] + 1
            ax2.plot(cycle_number, CE, linestyle='-', marker='o',
                     lw=line_width, label=None, color=colors[index],
                     markerfacecolor='w')
...
def plot_Efficiency(cell_names, plt: pyplot = None, ax: axes.Axes = None,
                    cycles=None, continuous=False, label_names=None, cmap=None,
                    palette=None, xlabel=None,
                    ylabel='Coulombic Efficiency (%)', xlim=None, ylim=None,
                    line_width=1.5, show_lgd=True, lgd_fsize=20, show_fig=True,
                    savefile=None, save_name=None, fsize=(6, 6)):
    if not plt:
        plt = pyplot
        # set font size
        plt.rcParams['font.size'] = lgd_fsize
    ...
    if not palette:
        for cell_name in cell_names:
            ...
            CE = df[CLABEL.CE.value]
            cycle_number = df[CLABEL.CYCLE_INDEX.value] + 1
            plt.plot(cycle_number, CE, linestyle='-', marker='o',
                     lw=line_width, label=cell_name, markerfacecolor='w')
    else:
        colors = list(sns.color_palette(palette, n_colors=len(cell_names)))
...
def plot_VoltTime(cell_name, plt: pyplot = None, ax: axes.Axes = None,
                  cycles=None, continuous=False, cmap=None, palette='flare',
                  xlabel=None, ylabel='Voltage (V)', xlim=None, ylim=None,
                  line_width=1.5, show_lgd=True, lgd_fsize=20, show_fig=True,
                  savefile=None, fsize=(6, 6)):
    df = get_csv(cell_name)
    ...
    plt.plot(df_time, df[LABEL.VOLTAGE.value], '-', color='black',
             lw=line_width)
...
def plot_dQdV(cell_name, plt: pyplot = None, ax: axes.Axes = None, cycles=None,
              continuous=False, cmap=None, palette='flare', xlabel=None,
              ylabel='dQ/dV (mAh/gV)', xlim=None, ylim=None, line_width=1.5,
              show_lgd=True, lgd_fsize=20, show_fig=True, savefile=None,
              fsize=(6, 6)):
    df = get_csv(cell_name)
    ...
    for i in clean_cycles:
        ...
        if i == first_cycle or i == last_cycle:
            ...
            label = None
        plt.plot(cycle_df[LABEL.VOLTAGE.value], cycle_df[LABEL.DQ_DV.value],
...
#         target_chg_columns = ['Q charge/mA.h','gravimetric Q charge (mAh/g)',
#                       'Energy charge/W.h','gravimetric energy charge (Wh/g)']
#         target_dchg_columns = ['Q discharge/mA.h',
#                                'gravimetric Q discharge (mAh/g)',
#                                'Energy discharge/W.h',
#                                'gravimetric energy discharge (Wh/g)']
#         # find the maximum or average value for various metrics for charge
#         # and discharge of each cycle
